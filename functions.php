...
if ( ! function_exists( 'devocraft_setup' ) ) :
	...
	function devocraft_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'devocraft' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'devocraft_custom_background_args',
				array(
 ...
 */
function devocraft_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'devocraft' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'devocraft' ),
...
add_action( 'widgets_init', 'devocraft_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function devocraft_scripts() {
	wp_enqueue_style( 'devocraft-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'devocraft-style', 'rtl', 'replace' );

	wp_enqueue_script( 'devocraft-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
